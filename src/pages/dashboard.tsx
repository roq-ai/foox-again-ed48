...
import { getCountries } from 'apiSdk/countries';
import { CountryInterface } from 'interfaces/country';
import { getFreelanceProfiles } from 'apiSdk/freelance-profiles';
import { FreelanceProfileInterface } from 'interfaces/freelance-profile';
import { getHirings } from 'apiSdk/hirings';
import { HiringInterface } from 'interfaces/hiring';

import { FreelanceProfileListPage as ShowCaseList } from 'pages/freelance-profiles';

function DashboardPage() {
  const { session } = useSession();

  const countryStat = useSWR<PaginatedInterface<CountryInterface>>(
    () => `/countries`,
    () => getCountries({ limit: 1, offset: 0 }),
  );

  const freelanceProfileStat = useSWR<PaginatedInterface<FreelanceProfileInterface>>(
    () => `/freelance-profiles`,
    () => getFreelanceProfiles({ limit: 1, offset: 0 }),
  );

  const hiringStat = useSWR<PaginatedInterface<HiringInterface>>(
    () => `/hirings`,
    () => getHirings({ limit: 1, offset: 0 }),
  );

  const statResults: { title: string; result: number; change: number; isLoading: boolean }[] = [
    {
      title: 'Countries',
      result: countryStat?.data?.totalCount,
      isLoading: countryStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Freelance Profiles',
      result: freelanceProfileStat?.data?.totalCount,
      isLoading: freelanceProfileStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
    {
      title: 'Hirings',
      result: hiringStat?.data?.totalCount,
      isLoading: hiringStat.isLoading,
      change: Math.floor(Math.random() * (100 - 40 + 1) + 40),
    },
            ...
            <Box>
              <Flex mb={4} justifyContent={'space-between'}>
                <Text as="b" fontSize="xl">
                  Latest Freelance Profiles
                </Text>
                <NextLink href={`freelance-profiles`} passHref legacyBehavior>
                  <Button
